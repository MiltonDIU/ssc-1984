...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('cruds.profession.title_singular') }}
        </div>
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.professions.update", [$profession->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $profession->name) }}" required>
                    @if($errors->has('name'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $profession->slug) }}">
                    @if($errors->has('slug'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('is_active') ? 'is-invalid' : '' }}" name="is_active" id="is_active" required>
                        ...
                        @foreach(App\Models\Profession::IS_ACTIVE_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('is_active', $profession->is_active) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('profession_parrent') ? 'is-invalid' : '' }}" type="number" name="profession_parrent" id="profession_parrent" value="{{ old('profession_parrent', $profession->profession_parrent) }}" step="1">
